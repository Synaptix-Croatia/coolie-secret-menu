...
(() => {
  ...
  // src/cdn.ts
  // Coolinarika production build: every fallback URL is same-origin (resolved against the page), so the runtime can
  // never request a third-party CDN. React/ReactDOM are the self-hosted copies of these exact pinned builds (SRI
  // unchanged). Babel is not shipped: it is only needed for JSX x-imports, which this page does not use.
